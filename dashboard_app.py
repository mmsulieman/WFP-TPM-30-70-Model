# dashboard_app.py (rosters-enabled + Excel export)
...
st.set_page_config(page_title="WFP–TPM Dashboard (30/70, Act3 full TPM, Rosters)", layout="wide")
st.title("WFP–TPM Dashboard")
st.caption("Somali Region | Allocation + KPIs + Charts + Map + Excel + PDF + Flow webhook + Named Rosters | Built for Jijiga AO")

with st.expander("Overview", expanded=False):
    st.markdown(
        """
        **Purpose:** Manage and visualize allocation of WFP and TPM responsibilities across activities, Woredas, distribution sites, and partners—**with named monitor rosters, capacity checks, Excel export, and PDF export**.

        **Model:** Non‑refugee activities follow **WFP 30% / TPM 70%**; **Refugees (Act 3)** are **full TPM** for routine monitoring. WFP may perform **random overnight spot‑checks** on Act 3.
        """
    )
...
scope = st.sidebar.selectbox(
    "Enforce WFP% by",
    ["Overall (non‑refugee)", "Per Woreda (non‑refugee)", "Per Woreda × Activity (non‑refugee)"]
)
...
# Sample rosters
@st.cache_data
def sample_roster_combined():
    return pd.DataFrame([
        {"monitor_name":"WFP-1","org":"WFP","capacity":25,"base_woreda":"Jijiga"},
        {"monitor_name":"WFP-2","org":"WFP","capacity":25,"base_woreda":"Awbare"},
        {"monitor_name":"WFP-3","org":"WFP","capacity":25,"base_woreda":"Shinile"},
        {"monitor_name":"WFP-4","org":"WFP","capacity":25,"base_woreda":"Kebri Beyah"},
        {"monitor_name":"WFP-5","org":"WFP","capacity":25,"base_woreda":"(any)"},
        {"monitor_name":"TPM-1","org":"TPM","capacity":40,"base_woreda":"Jijiga"},
        {"monitor_name":"TPM-2","org":"TPM","capacity":40,"base_woreda":"Awbare"},
        {"monitor_name":"TPM-3","org":"TPM","capacity":40,"base_woreda":"Shinile"},
        {"monitor_name":"TPM-4","org":"TPM","capacity":40,"base_woreda":"Kebri Beyah"},
        {"monitor_name":"TPM-5","org":"TPM","capacity":40,"base_woreda":"(any)"},
        {"monitor_name":"TPM-6","org":"TPM","capacity":40,"base_woreda":"(any)"},
        {"monitor_name":"TPM-7","org":"TPM","capacity":40,"base_woreda":"(any)"},
        {"monitor_name":"TPM-8","org":"TPM","capacity":40,"base_woreda":"(any)"},
    ])

@st.cache_data
def sample_roster_split():
    wfp = pd.DataFrame([
        {"monitor_name":"WFP-1","capacity":25,"base_woreda":"Jijiga"},
        {"monitor_name":"WFP-2","capacity":25,"base_woreda":"Awbare"},
        {"monitor_name":"WFP-3","capacity":25,"base_woreda":"Shinile"},
        {"monitor_name":"WFP-4","capacity":25,"base_woreda":"Kebri Beyah"},
        {"monitor_name":"WFP-5","capacity":25,"base_woreda":"(any)"},
    ])
    tpm = pd.DataFrame([
        {"monitor_name":"TPM-1","capacity":40,"base_woreda":"Jijiga"},
        {"monitor_name":"TPM-2","capacity":40,"base_woreda":"Awbare"},
        {"monitor_name":"TPM-3","capacity":40,"base_woreda":"Shinile"},
        {"monitor_name":"TPM-4","capacity":40,"base_woreda":"Kebri Beyah"},
        {"monitor_name":"TPM-5","capacity":40,"base_woreda":"(any)"},
        {"monitor_name":"TPM-6","capacity":40,"base_woreda":"(any)"},
        {"monitor_name":"TPM-7","capacity":40,"base_woreda":"(any)"},
        {"monitor_name":"TPM-8","capacity":40,"base_woreda":"(any)"},
    ])
    return wfp, tpm
...
# Required columns
required = [
    "Woreda","Distribution Site","Activity Code","Activity Name",
    "Total Planned Sites","Latitude","Longitude"
]
...
# Normalize partner column if present
partner_col = None
...
# Overnight spot-checks for Act3 per Woreda (dup as WFP)
spot_rows = []
if act3_spots > 0:
...
if spot_rows:
    ...
    exp = pd.concat([exp, spots], ignore_index=True)

# ===== Rosters & Monitor Assignment =====
monitor_load = None
...
if use_rosters:
    ...
    if roster_info == "Single combined file":
        if roster_file is not None:
            ...
            combined = sample_roster_combined()
        ...
        for col in ["monitor_name","org","capacity"]:
            if col not in combined.columns:
                st.error(f"Roster missing required column: {col}")
                st.stop()
        if "base_woreda" not in combined.columns:
            combined["base_woreda"] = "(any)"
        combined["org"] = combined["org"].str.upper().replace({"WFP":"WFP","TPM":"TPM"})
        roster_wfp = combined[combined["org"]=="WFP"][["monitor_name","capacity","base_woreda"]].copy()
        roster_tpm = combined[combined["org"]=="TPM"]["monitor_name"].to_frame()
        roster_tpm["capacity"] = combined[combined["org"]=="TPM"]["capacity"].values
        roster_tpm["base_woreda"] = combined[combined["org"]=="TPM"]["base_woreda"].values
    else:
        wfp_df, tpm_df = None, None
        if wfp_roster_file is not None:
            wfp_df = read_df(wfp_roster_file)
        else:
            wfp_df, _ = sample_roster_split()
            st.info("Using sample WFP roster (upload to override).")
        if tpm_roster_file is not None:
            tpm_df = read_df(tpm_roster_file)
        else:
            _, tpm_df = sample_roster_split()
            st.info("Using sample TPM roster (upload to override).")
        # normalize
        def norm(df0):
            df = df0.copy()
            rn = {c:c.lower() for c in df.columns}
            df.rename(columns=rn, inplace=True)
            for col in ["monitor_name","capacity"]:
                if col not in df.columns:
                    st.error(f"Roster missing required column: {col}")
                    st.stop()
            if "base_woreda" not in df.columns:
                df["base_woreda"] = "(any)"
            return df[["monitor_name","capacity","base_woreda"]]
        roster_wfp = norm(wfp_df)
        roster_tpm = norm(tpm_df)
    ...
    def assign_monitors(df_units: pd.DataFrame, roster: pd.DataFrame, prefer_base=True) -> pd.Series:
        if df_units.empty:
        ...
        if len(monitors_all) == 0:
            return pd.Series([None]*len(df_units), index=df_units.index)

        def cycle_for_woreda(w):
            if not prefer_base:
                return monitors_all
            preferred = [m for m in monitors_all if str(base_map.get(m,"(any)")).lower() == str(w).lower()]
            if not preferred:
                preferred = [m for m in monitors_all if str(base_map.get(m,"(any)")).lower() == "(any)"]
            return preferred + [m for m in monitors_all if m not in preferred]

        assignment = pd.Series(index=df_units.index, dtype=object)
        df_units_sorted = df_units.sort_values(["Woreda","Activity Code","Distribution Site"])  # stability
        for wor, sub in df_units_sorted.groupby("Woreda"):
            ...
            for idx in sub.index:
                ...
                while attempts < len(order):
                    ...
                    attempts += 1
                if chosen is None:
                    chosen = order[pos % len(order)]  # assign over capacity; flagged later
    ...
    wfp_mask = routine_mask & (exp["Assigned To"]=="WFP")
    tpm_mask = routine_mask & (exp["Assigned To"]=="TPM")

    exp.loc[wfp_mask, "Monitor Name"] = assign_monitors(exp.loc[wfp_mask], roster_wfp, prefer_base=prefer_base)
    exp.loc[tpm_mask, "Monitor Name"] = assign_monitors(exp.loc[tpm_mask], roster_tpm, prefer_base=prefer_base)
    ...
    if monitor_load["over_capacity"].any():
        st.warning("Some monitors exceed capacity. Consider adjusting capacity/roster or planned units.")
...
# Coverage Table (Woreda × Activity; Partner optional)
...
cov = (routine.groupby(cov_dims)['Assigned To']
...
width = 0.35
ax1.bar(x - width/2, cov['WFP'], width, label='WFP', color="#2E86C1")
ax1.bar(x + width/2, cov['TPM'], width, label='TPM', color="#58D68D")
ax1.set_xticks(x)
...
ax2.pie([wfp_units, tpm_units], labels=[f"WFP ({wfp_units})", f"TPM ({tpm_units})"], autopct='%1.1f%%', colors=["#2E86C1","#58D68D"])
...
if len(site_cov) > 0:
    midpoint = (np.mean(site_cov['Latitude']), np.mean(site_cov['Longitude']))
else:
    midpoint = (9.35, 42.79)
...
tooltip = {
    "html": "<b>{Distribution Site}</b><br/>Woreda: {Woreda}<br/>TPM %: {TPM %}<br/>Total: {Total}",
...
alloc_cols = [
    "Woreda","Distribution Site","Activity Code","Activity Name","Partner",
    "Latitude","Longitude","Assigned To","Modality","Monitor Name"
]
try:
...
excel_bytes = to_excel_bytes(
    allocations=allocations_export,
    coverage=cov,
    monitor_load=(monitor_load if monitor_load is not None else None),
)

st.download_button(
    "⬇️ Download Excel (allocations, coverage, monitor_load)",
    data=excel_bytes,
    file_name=f"wfp_tpm_dashboard_export_{dt.date.today().isoformat()}.xlsx",
...
if gen_pdf:
    ...
    tbl.scale(1, 1.2)
    tbl_buf = io.BytesIO()
    tbl_fig.savefig(tbl_buf, format='png', dpi=180, bbox_inches='tight')
    plt.close(tbl_fig)
    tbl_buf.seek(0)
    png_tbl = tbl_buf.read()

    # Monitor load image (if available)
    png_load = None
    if monitor_load is not None and len(monitor_load)>0:
        ...
        tbl2.scale(1, 1.1)
        load_buf = io.BytesIO()
        load_fig.savefig(load_buf, format='png', dpi=180, bbox_inches='tight')
        plt.close(load_fig)
        load_buf.seek(0)
    ...
    title = "WFP–TPM Dashboard Report"
    subtitle = f"Generated: {dt.datetime.now().strftime('%Y-%m-%d %H:%M')}"
    kpi_text = f"Total units: {total_units}\nWFP: {wfp_units} ({wfp_pct:.1f}%)\nTPM: {tpm_units} ({tpm_pct:.1f}%)\nAlert: {alert[0]}\n\nFilters:\nActivity: {', '.join(act_filter) if act_filter else 'All'}\nWoreda: {', '.join(wor_filter) if wor_filter else 'All'}\nSite: {', '.join(site_filter) if site_filter else 'All'}\nPartner: {', '.join(partner_filter) if partner_filter else 'All'}\nScope: {scope}\nWFP%: {wfp_share}%\nAct3 overnight per Woreda: {act3_spots}"
    ...
    page.insert_text((40,110), "KPIs & Filters", fontsize=14, fontname="helv")
    page.insert_text((40,130), kpi_text, fontsize=11, fontname="helv", fill=(0,0,0))
    page.insert_image(fitz.Rect(40, 240, 555, 470), stream=png1)
    page.insert_image(fitz.Rect(350, 480, 555, 700), stream=png2)
    ...
    # Optional: Send to Power Automate webhook
    if flow_url:
        try:
